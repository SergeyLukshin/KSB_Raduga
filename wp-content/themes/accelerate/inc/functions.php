...
function accelerate_scripts_styles_method() {
	...
	*/
	...
	wp_enqueue_style( 'lightbox-style', get_template_directory_uri() . 
               '/libs/lightbox/css/lightbox.css', array(), ' ' );

	wp_enqueue_script( 'lightbox-js', get_template_directory_uri() . 
               '/libs/lightbox/js/lightbox.js', array('jquery'), ' ' );

	wp_enqueue_style( 'dripicons', get_template_directory_uri() . 
               '/libs/dripicons/webfont/webfont.css', array(), ' ' );
...
function accelerate_footer_copyright() {
	$site_link = '<a href="' . esc_url( home_url( '/' ) ) . '" title="КСБ Радуга" ><span>КСБ Радуга</span></a>';
...
function inline_php($content){
    $content = preg_replace_callback('/\[php-inl\]((.|\n)*?)\[\/php-inl\]/', 'exec_php', $content);
    $content = preg_replace('/\[php-inl off\]((.|\n)*?)\[\/php-inl\]/', '$1', $content);
    return $content;
}
...
function myform_action_callback() {
	...
	$message="";
	$to="ksb@putevka.org"; // заменить на свою почту
	...
	if (empty($_POST['email']) && empty($_POST['mess'])) {
		$rtr='{"el1":"email", "el2":"mess", "error":""}';
		echo $rtr;
		exit;
	}

	if (empty($_POST['email'])) {
		$rtr='{"el1":"email", "el2":"", "error":""}';
		echo $rtr;
		exit;
	}
	if (empty($_POST['mess'])) {
		$rtr='{"el1":"mess", "el2":"", "error":""}';
		echo $rtr;
		exit;
	}
	...
	if(mail($to, $subject, $message, $headers)) {
		...
		$rtr='{"el1":"", "el2":"", "error":"Ошибка сервера."}';
	}
...
function myform_stylesheet(){
	wp_enqueue_style("myform_style_templ",get_bloginfo('stylesheet_directory')."/css/style_dlg_request.css","0.1.2",true);
	wp_enqueue_script("myform_script_temp",get_bloginfo('stylesheet_directory')."/js/ajax_dlg_request.js",array('jquery'),"0.1.2",true);
...
function formRequest() {
	$rty='<div class="col-md-12"><div class="request_label">Ваше имя</div></div>';
	$rty.='<div class="col-md-12"><div class="request_div_input request_div_padding"><input id="name" type="text" class="request_input" /></div></div>';
	$rty.='<div class="col-md-12"><div class="request_label">Адрес эл. почты*</div></div>';
	$rty.='<div class="col-md-12"><div id="email_div" class="request_div_input request_div_padding"><input id="email" type="text" class="request_input"/></div></div>';
	$rty.='<div class="col-md-12"><div class="request_label">Чем мы можем Вам помочь?*</div></div>';
	$rty.='<div class="col-md-12"><div id="mess_div" class="request_div_input"><textarea rows="5" id="mess" class="request_input"></textarea></div></div>';
	$rty.='<div class="col-md-12"><div class="btns"><input type="submit" class="input_close_box" value="Отменить"/>';
	$rty.='<input type="submit" class="input_ok_box" onclick="myform_ajax_send(\'#name\',\'#email\',\'#mess\'); return false;" value="Отправить"/></div></div>';
	return $rty;
}
add_shortcode( 'formRequest', 'formRequest' );

add_action( 'wp_enqueue_scripts', 'scripts_to_form' );
function scripts_to_form(){
    wp_enqueue_script("script_form", get_bloginfo( 'stylesheet_directory' ).'/js/open_dlg_request.js',array('jquery'),"0.1",false);
}
